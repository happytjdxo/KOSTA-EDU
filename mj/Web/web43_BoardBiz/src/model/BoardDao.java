...
public class BoardDao implements StringQuery {
	...
	// 	여기서는 DataSourceManager에서 그 일을 전담하고 있다... 달라진점은..
	...
	//게시판 글 작성하기
	public void postring(BoardVO vo) throws SQLException {
		Connection conn = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		try{
			conn = getConnection();
			// 이전에는 여기다 쿼리문을 일일이 작성 이부분을 메타데이타화 시켰다.
			...
			//System.out.println("작성한 postring :: " + vo);
	...
	public BoardVO getPostingByNo(int no) throws SQLException {
		...
		try{
			...
			rs = ps.executeQuery();
			if(rs.next()){
				vo = new BoardVO(no, rs.getString("title"),
						rs.getString("writer"),
	...
	public static void main (String[ ] args) throws Exception {
		...
		//BoardDao.getInstance().postring(new BoardVO("두번째 글", "하바리2", "0071", "게시판 두번째 글"));
	}
}
