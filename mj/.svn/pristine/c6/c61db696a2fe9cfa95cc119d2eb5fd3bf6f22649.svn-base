...
public class RegisterMemberServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			//out.println(name + " 님이 Cafe 회원 등록 성공");
		}catch(Exception e){
			//out.println(name + " 님이 Cafe 회원 등록 실패");
		}
		//out.close();
		list.add(dto);
		getServletContext().setAttribute("list", list);		// 바인딩
		
		// 페이지 이동..showOne.jsp
		// 지금 가입한 사람의 정보도 보여주고 / 전체명단을 볼 수 있는 링크도 함께.
